    ...
    <div className="fixed inset-0 z-[300] flex flex-col h-screen w-full bg-slate-50 overflow-hidden animate-fade-in">
      ...
      <header className="bg-slate-900 text-white p-4 md:p-6 md:px-12 flex items-center justify-between shadow-xl z-20">
        <div className="flex items-center gap-3 md:gap-6">
          <button onClick={() => !isSubmitting && navigate(-1)} className="p-2 md:p-3 hover:bg-white/10 rounded-full transition-all">
          ...
          <div className="flex items-center gap-3 md:gap-5">
            <div className="bg-indigo-600 p-3 md:p-4 rounded-xl md:rounded-2xl hidden sm:block">
              <FaCheckCircle className="text-xl md:text-3xl text-white" />
            </div>
            <div>
              <h2 className="text-lg md:text-3xl font-black tracking-tight leading-none">
                {isSuccess ? "¡Reserva Exitosa!" : "Confirmación de Turno"}
              </h2>
              <p className="text-indigo-400 font-bold uppercase text-[10px] md:text-sm tracking-[0.2em] mt-1 md:mt-2">
      ...
      <main className="flex-1 overflow-y-auto bg-slate-50 py-6 md:py-10 px-4 md:px-6 flex flex-col items-center">
        <div className="w-full max-w-3xl space-y-6 md:space-y-8">
          
          {isSuccess ? (
            <div className="bg-white p-8 md:p-12 rounded-[2rem] md:rounded-[3rem] border border-slate-200 shadow-xl text-center space-y-4 md:space-y-6 animate-slide-up">
              <div className="w-16 h-16 md:w-24 md:h-24 bg-green-100 text-green-600 rounded-full flex items-center justify-center mx-auto mb-2 md:mb-4">
                <FaCheckCircle className="text-4xl md:text-6xl animate-bounce" />
              </div>
              <h2 className="text-2xl md:text-4xl font-black text-slate-800 tracking-tight">¡Turno Reservado!</h2>
              <p className="text-slate-500 text-base md:text-lg font-medium">Te estamos redirigiendo a WhatsApp para avisar al profesional...</p>
            </div>
          ) : (
            <div className="space-y-6 animate-slide-up">
              {/* CARD DETALLES DEL TURNO */}
              <div className="bg-white p-6 md:p-10 rounded-[1.5rem] md:rounded-[2.5rem] border border-slate-200 shadow-sm space-y-6 md:space-y-8">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-y-5 md:gap-y-6 md:gap-x-12">
                  <div className="flex gap-4">
                    <div className="bg-slate-50 p-3 h-fit rounded-xl text-indigo-600 shrink-0"><FaCalendarAlt /></div>
                    <div>
                      <p className="text-[10px] font-black text-slate-400 uppercase tracking-widest">Fecha</p>
                      <p className="text-base md:text-lg font-bold text-slate-700 leading-tight">{formatearFechaSQL(selectedTurno?.fecha)}</p>
                    </div>
                  </div>
                  <div className="flex gap-4">
                    <div className="bg-slate-50 p-3 h-fit rounded-xl text-orange-500 shrink-0"><FaClock /></div>
                    <div>
                      <p className="text-[10px] font-black text-slate-400 uppercase tracking-widest">Horario</p>
                      <p className="text-base md:text-lg font-bold text-slate-700">{formatearHora(selectedTurno?.hora)} hs</p>
                    </div>
                  </div>
                  <div className="flex gap-4">
                    <div className="bg-slate-50 p-3 h-fit rounded-xl text-purple-600 shrink-0"><FaStethoscope /></div>
                    <div>
                      <p className="text-[10px] font-black text-slate-400 uppercase tracking-widest">Profesional</p>
                      <p className="text-base md:text-lg font-bold text-slate-700">{definirTitulo(profesional?.titulo)} {profesional?.nombre} {profesional?.apellido}</p>
                    </div>
                  </div>
                  <div className="flex gap-4">
                    <div className="bg-slate-50 p-3 h-fit rounded-xl text-red-500 shrink-0"><FaMapMarkerAlt /></div>
                    <div>
                      <p className="text-[10px] font-black text-slate-400 uppercase tracking-widest">Lugar</p>
                      <p className="text-base md:text-lg font-bold text-slate-700 leading-tight">{consultorio?.direccion}</p>
                      <p className="text-xs text-slate-400 font-bold uppercase mt-0.5">{consultorio?.localidad}</p>
                    </div>
                  </div>
                </div>
              </div>
              ...
              <div className="bg-white p-6 md:p-10 rounded-[1.5rem] md:rounded-[2.5rem] border border-slate-200 shadow-sm space-y-6 md:space-y-8">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4 md:gap-6">
                  <div className="bg-slate-50 p-4 md:p-5 rounded-xl md:rounded-2xl border border-slate-100">
                    ...
                    <p className="text-lg md:text-xl font-black text-slate-800 leading-tight">{formData?.apellido}, {formData?.nombre}</p>
                  </div>
                  <div className="bg-slate-50 p-4 md:p-5 rounded-xl md:rounded-2xl border border-slate-100">
                    <p className="text-[10px] font-black text-slate-400 uppercase tracking-widest mb-1">Documento (DNI)</p>
                    <p className="text-lg md:text-xl font-black text-slate-800">{formData?.dni}</p>
                  </div>
                  <div className="bg-slate-50 p-4 md:p-5 rounded-xl md:rounded-2xl border border-slate-100">
                    <p className="text-[10px] font-black text-slate-400 uppercase tracking-widest mb-1">Cobertura</p>
                    <p className="text-base md:text-lg font-bold text-indigo-600 uppercase">
                  ...
                  <div className="bg-slate-50 p-4 md:p-5 rounded-xl md:rounded-2xl border border-slate-100">
                    ...
                    <p className="text-base md:text-lg font-bold text-slate-800">{formData?.telefono}</p>
                  </div>
                </div>
              </div>
            </div>
          ...
          {/* MENSAJE DE ERROR */}
          {submitError && (
            <div className="p-4 md:p-6 bg-red-50 border-2 border-red-100 rounded-2xl md:rounded-3xl text-red-600 font-bold flex items-center gap-3 md:gap-4 text-sm md:text-base">
              <FaExclamationCircle className="text-xl md:text-2xl shrink-0" /> {submitError}
            </div>
          )}

          {/* FOOTER DE BOTONES */}
          <div className="flex flex-col md:flex-row gap-4 md:gap-5 pb-12 md:pb-20">
            {!isSuccess ? (
              <>
                <button
                  type="button" onClick={() => navigate(-1)} disabled={isSubmitting}
                  className="order-2 md:order-1 flex-1 py-4 md:py-5 px-8 bg-white border-2 border-slate-200 text-slate-500 rounded-xl md:rounded-2xl font-black tracking-widest hover:bg-slate-50 transition-all flex items-center justify-center gap-3 disabled:opacity-50 text-sm md:text-base"
                >
                  <FaAngleLeft /> EDITAR
                </button>
                <button
                  type="button" onClick={reservarTurno} disabled={isSubmitting}
                  className={`order-1 md:order-2 flex-[2] py-4 md:py-5 px-8 rounded-xl md:rounded-2xl font-black tracking-[0.15em] md:tracking-[0.2em] text-white shadow-xl transition-all flex items-center justify-center gap-3 text-sm md:text-base
                    ${isSubmitting ? 'bg-slate-400 cursor-not-allowed' : 'bg-slate-900 hover:bg-green-600 active:scale-[0.98]'}`}
                >
              ...
              <button
                onClick={() => navigate("/")}
                className="w-full py-5 md:py-6 bg-slate-900 text-white rounded-2xl md:rounded-3xl font-black tracking-[0.2em] md:tracking-[0.3em] hover:bg-indigo-600 shadow-2xl transition-all text-sm md:text-base"
              >
                IR AL INICIO
              </button>
            )}
