...
const UserFormModalHorizontal = ({ onSubmit }) => {
  ...
  return (
    <div className="fixed inset-0 z-[300] flex flex-col h-screen w-full bg-slate-50 overflow-hidden animate-fade-in">
      ...
      {/* HEADER: Pantalla Completa Estilo Indigo */}
      <header className="bg-slate-900 text-white p-6 md:px-12 flex items-center justify-between shadow-xl z-20">
        <div className="flex items-center gap-6">
          ...
          <div className="flex items-center gap-5">
            <div className="bg-indigo-600 p-4 rounded-2xl hidden md:block">
              <FaUser className="text-3xl text-white" />
            </div>
            <div>
              <h2 className="text-2xl md:text-3xl font-black tracking-tight leading-none">Confirma tus Datos</h2>
              <p className="text-indigo-400 font-bold uppercase text-xs md:text-sm tracking-widest mt-2">Paso final para tu turno</p>
            </div>
          </div>
        </div>
      ...
      <main className="flex-1 overflow-y-auto bg-slate-50 flex flex-col items-center py-10 px-6">
        <div className="w-full max-w-4xl space-y-8">
          
          <form onSubmit={handleSubmit} id="user-form" className="bg-white p-8 md:p-12 rounded-[2.5rem] border border-slate-200 shadow-sm space-y-10">
            ...
            <div className="space-y-6">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                <div className="space-y-2">
                  <label className="text-slate-500 text-xs font-black uppercase tracking-widest ml-1">Nombre *</label>
                  <input
                    type="text" name="nombre" value={formData.nombre} onChange={handleChange}
                    placeholder="Ej: Juan" required
                    className="w-full p-4 md:p-5 border-2 border-slate-100 rounded-2xl bg-slate-50 outline-none focus:ring-4 focus:ring-indigo-500/10 focus:border-indigo-500 transition-all font-bold text-lg text-slate-700"
                  />
                </div>
                <div className="space-y-2">
                  <label className="text-slate-500 text-xs font-black uppercase tracking-widest ml-1">Apellido *</label>
                  <input
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
              <div className="space-y-2">
                <label className="text-slate-500 text-xs font-black uppercase tracking-widest ml-1">DNI (7-8 dígitos) *</label>
                <div className="relative">
                  <FaIdCard className="absolute left-5 top-1/2 -translate-y-1/2 text-slate-300 text-xl" />
                  <input
                    type="text" name="dni" value={formData.dni} onChange={handleChange}
                    inputMode="numeric" maxLength="8" placeholder="34567890" required
                    className="w-full p-4 md:p-5 pl-14 border-2 border-slate-100 rounded-2xl bg-slate-50 outline-none focus:ring-4 focus:ring-indigo-500/10 focus:border-indigo-500 transition-all font-bold text-lg text-slate-700"
                  />
                </div>
              </div>

              <div className="space-y-2">
                <label className="text-slate-500 text-xs font-black uppercase tracking-widest ml-1">Teléfono (10 dígitos) *</label>
                <div className="relative">
                  <FaPhone className="absolute left-5 top-1/2 -translate-y-1/2 text-slate-300 text-xl" />
                  <input
            ...
            <div className="space-y-4">
              <label className="text-slate-800 text-xl font-black flex items-center gap-3 border-b border-slate-100 pb-4">
                <FaShieldAlt className="text-indigo-600" /> Cobertura Médica
              </label>

              {isLoadingOptions ? (
                <div className="p-8 text-center bg-indigo-50 rounded-2xl animate-pulse">
                  <p className="text-indigo-600 font-bold">Cargando coberturas disponibles...</p>
                </div>
              ) : errorOptions ? (
                <div className="p-4 bg-red-50 border-2 border-red-100 rounded-2xl text-red-600 font-bold flex items-center gap-3">
                  <FaExclamationCircle /> {errorOptions}
                </div>
              ) : (
                <select
                  name="selectedOption" value={formData.selectedOption} onChange={handleChange} required
                  className="w-full p-4 md:p-6 border-2 border-slate-100 rounded-2xl bg-slate-50 outline-none focus:ring-4 focus:ring-indigo-500/10 focus:border-indigo-500 transition-all font-black text-lg text-slate-700 appearance-none cursor-pointer"
                >
          ...
          <div className="flex flex-col md:flex-row gap-5 pb-20">
            <button
              ...
              className="flex-1 py-5 px-8 bg-white border-2 border-slate-200 text-slate-500 rounded-2xl font-black tracking-widest hover:bg-slate-50 hover:text-slate-800 transition-all flex items-center justify-center gap-3"
            >
              <FaAngleLeft /> VOLVER
            </button>
            <button
