...
const PanelConsultorioPropio = ({ perfilData: perfil, enviarMedicoID }) => {
  ...
  const { profesional: profesionalesObtenidos, isLoading: isLoadingProfesionales, fetchProfesional } = useObtenerProfesionalxIdPerfil(perfilID);
  ...
  useEffect(() => {
    if (!isLoadingProfesionales) {
      if (!medico) setShowModalAsociar(true);
      else setShowModalAsociar(false);
    }
  }, [medico, isLoadingProfesionales]);

  const { consultorios: consultoriosObtenidos, fetchConsultorio } = useObtenerConsultoriosxIdProfesional(medicoID);
  ...
  const handleCopyLink = () => {
    ...
    toast.info("¡Enlace de turnos copiado!", { icon: <FaCopy className="text-indigo-400" /> });
  };

  const countByEstado = (estado) => turnos?.filter(t => 
    new Date(t.fecha).toLocaleDateString('en-CA') === new Date().toLocaleDateString('en-CA') && t.estado === estado
  ).length || 0;

  const turnosFiltrados = turnos?.filter(t => 
    new Date(t.fecha).toLocaleDateString('en-CA') === fechaVisualizada.toLocaleDateString('en-CA') && t.estado === "reservado"
  ...
  return (
    <div className="min-h-screen py-4 sm:py-12 px-2 sm:px-6 bg-slate-50">
      <div className="max-w-7xl mx-auto space-y-6 sm:space-y-12">
                ...
                <div className="flex flex-col sm:flex-row items-start sm:items-center gap-4 sm:gap-8">
                  ...
                  <div className="flex-1">
                    ...
                    <div className="inline-flex items-stretch bg-slate-800 border border-slate-700 rounded-xl overflow-hidden shadow-lg">
                        ...
                        >
                            <FaExternalLinkAlt size={10} className="text-slate-400" /> Ver Link
                        </button>
                        <button 
                            onClick={handleCopyLink}
                            className="flex items-center gap-2 px-4 py-2 text-[10px] font-black uppercase hover:bg-indigo-600 transition-colors bg-indigo-500/10 text-indigo-400"
                        >
              ...
              <div className="max-h-40 overflow-y-auto space-y-2 custom-scrollbar pr-1">
                {consultoriosObtenidos?.map((c) => (
                  ...
                  >
                    ...
                    <div className="flex-1 text-left min-w-0 text-[10px] font-black uppercase truncate">{c.nombre}</div>
                  </button>
                ))}
              </div>
              <button onClick={() => setShowModalCrearConsultorio(true)} className="w-full mt-3 py-2 border border-dashed border-slate-600 text-[8px] font-black rounded-lg text-slate-500 uppercase">
        ...
        {/* GRID DE ESTADÍSTICAS Y ACCIONES CON COLORES DIFERENCIADOS */}
        <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-5 gap-3 sm:gap-6">
          <StatCard label="Pacientes" value={countByEstado("reservado")} icon={FaCalendarAlt} color="text-slate-400" />
          ...
          {/* BOTÓN COBERTURAS - COLOR PÚRPURA */}
          <StatCard 
            label="COBERTURAS" 
            value="GESTIÓN" 
            icon={FaShieldAlt} 
          ...
          {/* BOTÓN AGENDA - COLOR INDIGO/BLUE */}
          <StatCard 
            label="AGENDA" 
            value="TURNOS" 
            icon={FaCalendarAlt} 
        ...
        <section className="bg-white rounded-[2rem] p-5 sm:p-10 shadow-lg border border-slate-100">
           ...
           <div className="flex flex-col sm:flex-row items-center justify-between mb-8 gap-4">
                ...
                <div className="flex gap-1 bg-slate-100 p-1 rounded-xl">
                  <button onClick={() => {const d = new Date(fechaVisualizada); d.setDate(d.getDate()-1); setFechaVisualizada(d)}} className="p-2"><FaChevronLeft size={12}/></button>
                  <button onClick={() => setFechaVisualizada(new Date())} className="px-3 text-[9px] font-black uppercase">Hoy</button>
                  <button onClick={() => {const d = new Date(fechaVisualizada); d.setDate(d.getDate()+1); setFechaVisualizada(d)}} className="p-2"><FaChevronRight size={12}/></button>
                </div>
           </div>
           ...
           {isLoadingTurnos ? (
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3">
                {turnosFiltrados.map((turno) => (
                  <div key={turno.id} className="p-4 rounded-2xl bg-white border border-slate-100 flex items-center justify-between">
                    <div className="flex items-center gap-3">
                      <div className="bg-slate-900 px-3 py-1 rounded-lg text-white font-black text-[10px]">{turno.hora.slice(0,5)}</div>
                      <div className="font-black text-[10px] uppercase">{turno.apellido_paciente}, {turno.nombre_paciente}</div>
                    </div>
                  </div>
                ))}
              </div>
           ) : (
              <div className="py-10 text-center text-slate-400 text-xs font-black uppercase italic border-2 border-dashed border-slate-100 rounded-xl">Sin pacientes</div>
           )}
        </section>
      </div>
...
// COMPONENTE STATCARD ACTUALIZADO PARA ARMONÍA Y COLOR
const StatCard = ({ label, value, icon: Icon, color, onClick, clickable, bgHover }) => (
  <div 
    onClick={clickable ? onClick : undefined} 
    className={`p-4 rounded-2xl bg-white border border-slate-100 flex flex-col items-center justify-center transition-all group ${
      clickable ? `cursor-pointer ${bgHover || "hover:border-slate-300"} hover:shadow-md active:scale-95` : ""
    }`}
  >
    <Icon className={`${color} mb-2 transition-transform group-hover:scale-110`} size={18} />
    <span className={`text-xl font-black uppercase ${clickable ? color : "text-slate-900"}`}>{value}</span>
