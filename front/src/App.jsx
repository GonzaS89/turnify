...
import TurnAsignedModal from './Layouts/components/TurnAsignedModal'; // Importa el modal de turno asignado
...
const App = () => {
    ...
    const [showTurnAsignedModal, setShowTurnAsignedModal] = useState(false); // Estado para mostrar el modal de turno firmado
    ...
    const handleConfirmBooking = () => {
        ...
        setFechaSeleccionada(''); // Limpia la fecha seleccionada al confirmar la reserva
        setShowUserFormModal(false); // Cierra el modal de formulario de usuario
        setShowConfirmationModal(false); // Cierra el modal de confirmación
        setShowTurnAsignedModal(true)
        cerrarModalTurnos(true); // Cierra todos los modales
     

        // Considera reemplazar esto con un modal de éxito más elegante
    ...
    <div className="fixed inset-0 bg-black bg-opacity-75 flex items-center justify-center z-50 backdrop-blur-sm p-4 sm:p-6">
        {/* Contenedor principal del modal - Ajustado para responsividad */}
        <div className={`bg-white rounded-xl shadow-2xl p-4 sm:p-6 w-full max-w-sm sm:max-w-md md:max-w-lg lg:max-w-2xl flex flex-col gap-4 sm:gap-6
                        max-h-[95vh] overflow-y-auto`}> {/* Altura máxima y scroll */}
            {/* Información del Consultorio - Nueva Sección */}
            {consult && (
                <div className="text-center pb-3 sm:pb-4 border-b border-gray-200"> {/* Ajuste de padding */}
            ...
            {isLoadingTurnos ? (
                ...
                <div className="bg-red-100 border border-red-400 text-red-700 px-3 py-2 rounded-lg text-center text-sm sm:text-base"> {/* Ajuste de padding */}
            ...
            >
                ...
                {showConfirmationModal && (
                    <ConfirmationModal
                        ...
                        turnoReservado={handleConfirmBooking}
                ...
                { /* Modal de Turno Asignado */}
                {showTurnAsignedModal && (
                    <TurnAsignedModal
                        isOpen={showTurnAsignedModal}
                        onClose={() => {
                            setShowTurnAsignedModal(false);
                            cerrarModalTurnos(); // Cierra todos los modales al cerrar este
                        }}
                    />
                )}    
